...
from  test_par_inject import get_test
torch.set_default_tensor_type('torch.cuda.FloatTensor')

from encoder import get_encoder
# from autoencoder import *
from decoder_par_inject import get_decoder
...
decoder = get_decoder(output_size=output_size)
...
def train_step(tokens_tensor, image_tensor, decoder, decoder_optimizer, encoder, criterion):
    ...
    for seq in range(sequence_length - 1):
        ...
        decoder_output, decoder_hidden = decoder(input_tensor, decoder_hidden, feature_tensor) # , feature_tensor # seq
...
def train(encoder, decoder, batch_size=128, n_iters=50):
    plot_losses = []
    ...
    for iters in tqdm(range(1, n_iters + 1)):
        for ids, im_path in tqdm(enumerate(train_image_names)):
            ...
            image_batch[batch_index] = image # image
            caption_batch[batch_index] = train_captions_tokens[ids]

            if ids == len(train_image_names) - 1:
                loss = train_step(tokens_tensor=caption_batch[:batch_index + 1],
                                  image_tensor=image_batch[:batch_index + 1], decoder=decoder,
                                  decoder_optimizer=decoder_optimizer, encoder=encoder, criterion=criterion)
                print_loss_total += loss
                batch_index = 0
                break

            if batch_index == batch_size - 1:
                loss = train_step(tokens_tensor=caption_batch, image_tensor=image_batch, decoder=decoder,
                                  decoder_optimizer=decoder_optimizer, encoder=encoder, criterion=criterion)
                print_loss_total += loss
        ...
        torch.save(decoder, f"par_inject_1_layer_{iters}.pth")
        get_test(iters)
