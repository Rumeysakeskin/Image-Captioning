...
def train_step(tokens_tensor, image_tensor, decoder, decoder_optimizer, encoder, criterion):
    ...
    for seq in range(sequence_length - 1):
        ...
        decoder_output, decoder_hidden = decoder(input_tensor, decoder_hidden, feature_tensor, seq)
...
def generate_caption(image_tensor, encoder, decoder, max_len=30):
    ...
    for i in range(max_len):
        out, decoder_hidden = decoder(decoder_input, decoder_hidden, feature, i)
...
def train(encoder, decoder, batch_size=128, n_iters=50):
    ...
    for iters in tqdm(range(1, n_iters + 1)):
        ...
        torch.save(decoder, f"pre_inject_1_layer_{iters}.pth")
