...
from test_init_inject import get_test
from decoder_init_inject import get_decoder
...
def train_step(tokens_tensor, image_tensor, decoder, decoder_optimizer, encoder, criterion):
    ...
    for seq in range(sequence_length - 1):
        ...
        decoder_output, decoder_hidden = decoder(input_tensor, decoder_hidden)
...
def train(encoder, decoder, batch_size=128, n_iters=40):
    plot_losses = []
    ...
    for iters in tqdm(range(1, n_iters + 1)):
        ...
        torch.save(decoder, f"init_inject_1_layer_{iters}.pth")
